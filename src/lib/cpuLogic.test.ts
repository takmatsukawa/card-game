...
function createTestPlayer(id: number, mana: number = 10): Player {
	const testCards: Card[] = [
		...
		},
		{
			...
			hp: 3,
			commands: [
				{ manaCost: 1, damage: 3, description: '素早い攻撃' }
			]
		},
		{
			id: 3,
			...
			hp: 10,
			commands: [
				{ manaCost: 3, damage: 8, description: '強力な攻撃' }
			]
		},
		{
			id: 4,
...
function getPlayableCards(player: Player): MonsterCard[] {
	return player.hand.filter(card => 
		card.type === 'monster' && player.mana >= 1
	) as MonsterCard[];
}
...
function findBestPosition(emptyCells: { row: number; col: number }[]): { row: number; col: number } {
	// 戦略的なポジショニング: 角を優先
	const cornerPositions = emptyCells.filter(cell => 
		(cell.row === 0 || cell.row === 1) && (cell.col === 0 || cell.col === 1)
	);
...
function executeCPUTurn(cpu: Player, opponent: Player): { 
	cardsPlaced: number; 
	manaUsed: number; 
